...
name <- "Alexander Tran"
github.name <- "ExandTran"
email.address <- "exand.tran@berkeley.edus"
...
set.seed(42)
#z <- <your code here>
z<- sample(c(TRUE,FALSE),5000,prob=c(.3,.7),replace=TRUE)
...
# i.e. each element is one of the numbers 1 through 6, with equal probability.
  set.seed(31415)
#v <- <your code here>
v<- sample(seq(1,6),100,replace=TRUE)
...
#cw4 <- <your code here>
cw4<- ChickWeight[sample(seq(1,578),200),] 
...
# 
# t2 <- <your code here>
t2<- table(infants[infants$ed=="College",]$smoke)
...
bdiff<- mean(infants[infants$age>=35,]$bwt,na.rm=TRUE)-mean(infants[infants$age<35,]$bwt,na.rm=TRUE)
...
abline(lm(formula = mtcars$qsec ~ mtcars$hp))
# [3 pt]
...
plot(mtcars$wt,mtcars$disp,xlab="Weight in lb/1000",ylab="Engine Displacement in cu. in.",col=as.numeric(mtcars$am))
...
plot(WorldBank$fertility.rate,
     WorldBank$life.expectancy,
     pch=".",
     col=as.numeric(WorldBank$region),
     xlab="Fertility Rate",
...
plot(Bank1960$fertility.rate,
     Bank1960$life.expectancy,
     pch=".",
     col=as.numeric(Bank1960$region),
     xlab="Fertility Rate",
     ylab="Life Expectancy",
     main="1960"
)
...
par(mfrow=c(1,1))
hist(WorldBank[WorldBank$lending=="IDA",]$GDP.per.capita.Current.USD,
     breaks=max(WorldBank[WorldBank$lending=="IDA",]$GDP.per.capita.Current.USD,na.rm=TRUE)/250,
     xlab="GDP Per Capita in USD",
     main="GDP per capita where lending rating is IDA")
...
maxabsdiff<-function(x){
  return(max(abs(diff(x))))
}
max.diff.rain<- sapply(rain,maxabsdiff)
...
#    Now select at random k individuals from the whole sample (i.e. all n+m observations)
...
PermDiff <- function(cases, controls, k=5000){
   output=c()
   for (i in 1:k){
     newcases=sample(c(cases,controls),k)
     newcontrols=c()
     for (i in 1:length(controls)){
       if (controls[i] %in% cases==FALSE){
         newcontrols=append(newcontrols,controls[i])
       }
     }
     output=append(output,mean(newcases)-mean(newcontrols))
   }
   return(output)
}
...
NumJackpot <- function(k, B){
  n.jackpot=c()
  for (i in 1:B){
    winning.numbers=sample(seq(1,19),3)
    count=0
    tickets=replicate(k,list(sample(seq(1,19),3)))
    for (i in 1:length(tickets)){
      if (tickets[[i]][1] %in% winning.numbers & tickets[[i]][2] %in% winning.numbers & tickets[[i]][3] %in% winning.numbers){
        count=count+1
      }
    }
    n.jackpot=append(n.jackpot,count)
  }
  return(n.jackpot)
}
