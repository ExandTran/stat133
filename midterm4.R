...
dice_sum <- function(k=2, B=100){
  lst=c()
  for (i in 1:B){
    lst=append(lst,sum(sample(1:6,k)))
  }
  return(lst)
}
...
for (i in 1:length(phrases)){
  if (substring(phrases[i],str_length(phrases[i])-1,str_length(phrases[i])-1)=="o"){
    text1=append(text1,phrases[i])
  }
}
...
text2=c()
for (i in 1:length(phrases)){
  if (substring(phrases[i],1,1)=="d"){
    text2=append(text2,phrases[i])
  }
}
...
#even <- <your code here>
even <- paste0("even",seq(2,1000,by=2))
...
hotelCal.split=hotelCal.split[[1]][hotelCal.split[[1]]!=""]
...
abbreviate <- function(vector, k){
  ...
  for (i in 1:length(vector)){
    lst=append(lst,substring(vector[i],1,k))
  }
  return(lst)
}
