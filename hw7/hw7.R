...
presidents <- speeches[breaks+3][1:n.speeches]
...
Mo <- function(dates){
  return(substr(dates,str_length(dates)-7,str_length(dates)-6))
}

speechYr <-  as.numeric(substring(tempDates,str_length(tempDates)-3)[1:n.speeches])
speechMo <-  as.numeric(lapply(tempDates,Mo)[1:n.speeches])
...
index=c(breaks,length(n.speeches)+1)
for(i in 1:n.speeches){
  text=paste(speeches[(index[i]+6):(index[i+1]-2)],collapse= " ")
  speechesL[[i]]=unlist(str_split(text, "[.?!]")) 
...
# Output : words, a character vector where each element is one word 
  ...
  sentences=gsub("[0-9]|Applause.|Laughter.|", "", sentences)
  ...
  sentences=unlist(str_split(sentences, " "))
  sentences=unlist(str_split(sentences, "[[:punct:]]"))
...
count<- function(speeches){
  ...
  emptyVec1=emptyVec
  emptyVec1[names(speeches)] <- speeches
  return(emptyVec1)
}
...
# wordCount <- <your code here>
  wordCount <- matrix(
    unlist(wordVecs),
    ncol=length(speechesL),
    nrow=no.uniqueWords,
    byrow=FALSE)
wordMat = wordCount
...
speechesDF$speechYr=speechYr
speechesDF$speechMo=speechMo
...
presidentWordCount <- matrix(nrow=no.uniqueWords,ncol=length(unique.presidents))
for(i in 1:length(unique.presidents)){
  president.names=unique.presidents[i]
  correct = (president.names==presidents)
  data = wordCount[,correct]
  if(is.matrix(data)){
    president.words = rowSums(data)
  }
  presidentWordCount[,i]=president.words
}
presidentWordMat=presidentWordCount
...
docFreq<-c()
for(i in 1:nrow(presidentWordCount)){
  n=0
  for(j in 1:ncol(presidentWordCount)){
    if (presidentWordCount[i,j]>0){
      n=n+1
    }
  }
  docFreq=append(n,docFreq,after=0)
}
...
index.presidents=c()
certain.presidents=c()
for (i in 1:length(speechesDF$Pres)){
  if ((speechesDF$Pres[i] %in% certain.presidents)==FALSE){
    certain.presidents=append(levels(factor(speechesDF$Pres[i])),certain.presidents,after=0)
    index.presidents=append(i,index.presidents,after=0)
  }
}
presParty <- speechesDF$party[index.presidents]
...
plot(x=speechYr,y=sentences,main="Sentences",xlab="Years",ylab="Amount",pch=18)
plot(x=speechYr,y=words,main="Words",xlab="Years",ylab="Amount",pch=18)
plot(x=speechYr,y=chars,main="Characters",xlab="Years",ylab="Amount",pch=18)
plot(x=speechYr,y=(chars/words),main="Average Word Length",xlab="Years",ylab="Amount",pch=18)
plot(x=speechYr,y=(words/sentences),main="Average sentence length",xlab="Years",ylab="Amount",pch=18)
